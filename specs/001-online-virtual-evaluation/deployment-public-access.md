...
> ⚠️ **현재 미사용 (2026-07-30)**: 이 문서는 도메인 없이 무료로 공개하던 시기의 검토·결정 기록이다.
> 지금은 클라우드 서버 + 구입 도메인(`spg-deepracer.doublejeong.com`) + Caddy의 Let's Encrypt
> 인증서로 공개한다 — [cloud-migration.md](cloud-migration.md) 참고.
> Quick Tunnel은 **노트북이 절전·종료되면 주소가 영구히 사라진다**는 문제가 실제로 두 번 발생했고,
> 그것이 클라우드 이전을 결정한 직접적 계기였다. 비상 복구 수단으로서의 절차는 여전히 유효하므로
> 지우지 않고 남겨둔다.
