...
**001(온라인 가상 트랙 평가)을 먼저 전체 구현하고, 002(오프라인 비전 타이머)는 그 다음에 진행한다.** 온라인 서비스는 지금 있는 자원(서버 + DRFC)만으로 바로 시작할 수 있고, 001의 팀·시즌·리더보드 기반 위에 002가 얹히는 구조이기 때문.

> 당초 이 순서의 이유는 "오프라인은 카메라 등 물리 장비 조달이 필요해 시간이 걸린다"였다. **공기계 스마트폰을 쓰기로 결정해 조달 부담은 사라졌지만**, 002의 남은 선행 조건은 장비가 아니라 **대회 규정 확정(이탈 판정 기준·페널티 상한)과 트랙 현장 실측**이다 — [002 spec.md](specs/002-offline-vision-timer/spec.md) §10 참조.
...
- 오프라인 본선: 2바퀴 고정(1바퀴·2바퀴 구간기록 각각 저장), 연습주행(미반영)·실제주행(반영) 구분, 실제주행은 10분 내 무제한 재도전, **트랙 이탈 횟수 × 5초** 페널티(자동 집계 후 임원진이 확정), 차량·주행순서는 추첨.
- 오프라인 계측 장비: **공기계 스마트폰 2대**로 역할 분리 — 출발선 전용(랩타임)과 트랙 전체(이탈 감지·영상). 천장 설치가 불가능해 삼각대·스탠드 거치를 전제한다. 상세는 [002 plan.md](specs/002-offline-vision-timer/plan.md).
- 기술 스택(STEP2 확정): **Python 기반**(백엔드 FastAPI + 서버 렌더링) + **PostgreSQL** + 로컬 디스크 저장. 현재 실행 환경은 **두 대로 나뉜다** — 웹·DB는 AWS Lightsail(서울), 평가 워커·DRFC는 **AWS EC2 스팟 인스턴스**(m7i.xlarge, 4vCPU/16GB, GPU 없음)에서 돈다. 평가 1건당 실측 약 8분(노트북 시절 약 14분). 구축·운영 절차는 [worker-server-setup.md](docs/worker-server-setup.md). 운영자 노트북은 **예비 워커**로 두고 필요할 때만 켠다. 비전 타이머는 **공기계 스마트폰 2대 + 현장 노트북**으로 구성한다(전용 장비 구매 없음).
